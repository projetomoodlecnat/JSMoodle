    ...
    $.ajax(cookiesDict["api_Path"] + "dbproperties?index=" + cookiesDict["databaseIndex"], {
        ...
        success: function (firstStep) {
            ...
            $.post({
                    ...
                    try {
                        ...
                        if (optionType == "multichoice" || optionType == "truefalse") {
                            ...
                            while (questionToHTML.length > 0) {
                                stringHTMLBuilder += "<div id='questiontext-" + data[i][data[0].indexOf("SLOTID")] + "-" + k.toString() + "'>" + questionToHTML.pop() + "</div><select slot='" + data[i][data[0].indexOf("SLOTID")] + "'><option value=''></option>";
                ...
                $("#content").html(stringHTMLBuilder + "<br><br><br><p class='paragraphFinishAttempt'>FINALIZAR A SUA TENTATIVA</p>");
    ...
    switch (activityType) {
        case 'quizz':
            ...
            requiredFieldCheck.each(function () {
                var field = $(this);
                if ((field.val() != "" && field.prop('type') == 'TEXTBOX') || (field[0].checked || field.prop('type') == 'TEXTAREA' || field.prop('type') == 'select-one')) {
            ...
            if (matchSelected.length == 0) {
                ...
                $.each(matchSelected, function (index) {
                    var selectedElement = $(this);
                    var rightAnswerAndValue = ["", -9999.0];
                    var sumFractions = 0.0;
                    var isEssay = (selectedElement.prop('nodeName') == "TEXTAREA");
                    var isSelect = (selectedElement.prop('type') == "select-one");
                    $.post({
                        url: cookiesDict["api_Path"] + "selector" + cookiesDict["databaseType"],
                        async: false,
                        data: { "connectionIndex": connectionIndex, "query": command + " where mdl_quiz_slots.id=" + selectedElement.attr("slot") + " order by mdl_question_answers.id" }
                        ...
                        try {
                            if (isSelect) {
                                if (processedSlots.indexOf(selectedElement.attr('slot')) == -1) {
                                    var i = 1;
                                    $.post({
                                        url: cookiesDict["api_Path"] + "selector" + cookiesDict["databaseType"],
                                        async: false,
                                        data: { "connectionIndex": connectionIndex, "query": "select * from mdl_qtype_match_subquestions where questionid=" + data[i][data[0].indexOf("QUESTIONID")] }
                                    }).done(function (innerData, textStatus, jqXHR) {
                                        var selectMaxFraction = parseFloat(data[i][data[0].indexOf("MAXMARK")]);
                                        var errorPenalty = parseFloat(data[i][data[0].indexOf("PENALTY")].replace(/,/, '.'));
                                        var subquestionsHash = [];
                                        for (var j = 0; j < innerData.length; j++) {
                                            if (innerData[j][innerData[0].indexOf("QUESTIONTEXT")] != "") {
                                                subquestionsHash[innerData[j][innerData[0].indexOf("QUESTIONTEXT")]] = innerData[j][innerData[0].indexOf("ANSWERTEXT")];
                                            }
                                        }
                                        var k = 0;
                                        index = index - 1;
                                        while (matchSelected.eq(index + 1).length != 0 && matchSelected.eq(index + 1).prop('nodeName') == "SELECT") {
                                            index = index + 1;
                                            var questionText = $('#questiontext-' + data[i][data[0].indexOf("SLOTID")] + '-' + k.toString()).html();
                                            if (subquestionsHash[questionText] == "undefined") {
                                                // tratar erro e parar execução
                                                console.log("error!");
                                            }
                                            else if (matchSelected.eq(index).val() != subquestionsHash[questionText]) {
                                                selectMaxFraction = selectMaxFraction - errorPenalty;
                                            }
                                            fieldAnswer = fieldAnswer + questionText + "->" + matchSelected.eq(index).val() + ";";
                                            k++;
                                        }
                                        processedSlots.push(selectedElement.attr('slot'));
                                    }).error(function () {
                                        setStatus("error", "Erro na requisição à API.");
                                        isDone = false;
                                        return false;
                                    });
                                } else if (matchSelected.eq(index + 1).length == 0) {
                                    isDone = processUnansweredQuestions(questionUsageID, connectionIndex, command, layoutSlots);
                                }
                            } else if (isEssay) {
                                fieldAnswer = selectedElement.val();
